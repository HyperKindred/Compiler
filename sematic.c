...
switch (T->kind){
    case EXT_DEC_LIST:
        ...
        T->ptr[2]->type=T->type;              //将类型属性向下传递变量结点
        T->ptr[2]->offset=T->offset+T->width; //外部变量的偏移量向下传递
        T->ptr[2]->width=T->width;
        ext_var_list(T->ptr[0]);
        ext_var_list(T->ptr[2]);
        T->num=T->ptr[2]->num+1;
        break;
    case ID:                                     //最后一个变量名
	...
	switch (T->kind)
	{
	    ...
	    case ASSIGNOP:
            ...
            else
            {
                ...
                T->offset+=T->ptr[0]->width; //如果左值是下标变量，with会大于0
  ...
  if (T)
	...
	switch (T->kind)
	{
	    ...
	    case PARAM_LIST:    //处理函数形式参数列表
            ...
            break;
	    case  PARAM_DEC:
            rtn=fillSymbolTable(T->ptr[0]->type_id,newAlias(),1,T->ptr[1]->type,'P',T->offset);
            if (rtn==-1)
                semantic_error(T->ptr[1]->pos,T->ptr[1]->type_id, "参数名重复定义");
            else T->ptr[1]->place=rtn;
            T->num=1;       //参数个数计算的初始值
            T->width=T->ptr[1]->type==INT?4:8;  //参数宽度
	    ...
	    case STM_LIST:
                ...
                {     //2条以上语句连接,S.next属性向下传递
                    ...
                    if (T->ptr[1]->width>T->width) //后续语句需要更多单元时，更新这些
                        T->width=T->ptr[1]->width; //顺序语句的单元width，实现共享单元方式
                }
                break;
	    case IF_THEN:
